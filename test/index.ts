...
test.failing('.get() - `schema` option - default', t => {
	const store = new Conf({
		cwd: temporaryDirectory(),
		schema: z.object({
			foo: z.boolean().default(true),
			nested: z.object({
				bar: z.number().default(55)
			})
		})
	});
